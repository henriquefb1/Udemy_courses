...
produto_max = max(lista_compras, key=lambda produtos:produtos['preco'])
produto_min = min(lista_compras, key=lambda produtos:produtos['preco'])
print(f'O produto mais caro é {produto_max["nome"].title()} - R${produto_max["preco"]}')
print(f'O produto mais barato é {produto_min["nome"].title()} - R${produto_min["preco"]}')